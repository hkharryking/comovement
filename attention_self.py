...
class Attention_SELF(Attention_CNNE):
    """
    ...
    def __init__(self,feature_num,filters_num,batch_size,window,timestamps,sector_cardinality,d,da):
        super(Attention_SELF, self).__init__(feature_num, filters_num, batch_size, window, timestamps, sector_cardinality, d, da)
        self.lstm = nn.LSTM(
                                input_size = 1,
        ...
        self.W1=Variable(torch.randn(da,HIDDEN_UNIT*2)).cuda()
    ...
    def forward(self, x):
        ...
        attentions = []
        for seq in l_out:
            A = torch.mm(self.W2.transpose(0, 1), self.tanh(torch.mm(self.W1, seq.transpose(0,1))))
            attentions.append(self.softmax(A))
        attentions = torch.stack(attentions, dim=1).squeeze()
        return attentions, self.line(attentions)



    def init_hidden(self):
        # The axes semantics are (num_layers, minibatch_size, hidden_dim)
        h0 = Variable(torch.zeros(HIDDEN_LAYER*2, self.batch_size, HIDDEN_UNIT), requires_grad=True).cuda().float()
        c0 = Variable(torch.zeros(HIDDEN_LAYER*2, self.batch_size, HIDDEN_UNIT), requires_grad=True).cuda().float()
