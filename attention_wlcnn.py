...
class Attention_WLCNN(Attention_CNNE):
    """
    Linear weighted version
    """
    def get_code(self):
        return 'WLCNN'

    def __init__(self,feature_num,filters_num,batch_size,window,timestamps,sector_cardinality,d,da):
        super(Attention_WLCNN, self).__init__(feature_num,filters_num,batch_size,window,timestamps,sector_cardinality,d,da)
        self.lstm = nn.LSTM(
                                input_size = 1,
        ...
        self.line = nn.Linear(timestamps - window * 3 + 3, sector_cardinality)
    ...
    def forward(self, x):
        ...
        for seq in l_out:
            ...
            attentions.append(self.softmax(torch.mm(seq,(self.line.weight*A).transpose(0,1)).squeeze()))
